...
class ServiceManager(IServiceManager):
    ...
    async def register_service(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to register service {service_definition.name} for plugin {plugin_name}: {e}"
            )
            return False
    ...
    async def unregister_service(self, plugin_name: str, service_name: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to unregister service {service_name} for plugin {plugin_name}: {e}"
            )
            return False
...
class PluginEventSubscriptionManager(IPluginEventSubscriptionManager):
    ...
    def __init__(self, logger: logging.Logger | None = None):
        ...
        self.subscriptions: dict[
            str, list[PluginSubscriptionBase]
    ...
    async def subscribe(
        ...
        subscription = PluginSubscriptionBase(
            plugin_name=plugin_name, event_type=event_type, handler=handler
        )
    ...
    async def unsubscribe(self, plugin_name: str, event_type: str) -> bool:
        ...
        if event_type in self.subscriptions:
            self.subscriptions[event_type] = [
                sub for sub in self.subscriptions[event_type] if sub.plugin_name != plugin_name
            ]

        if (
            plugin_name in self.plugin_subscriptions
            and event_type in self.plugin_subscriptions[plugin_name]
        ):
...
class PluginManager(IPluginManager):
    ...
    async def start_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            if not hasattr(plugin, "context") or (
                hasattr(plugin, "context") and plugin.context is None
            ):
                context = PluginContext(
                    plugin_id=plugin_name, registry=self.registry, event_bus=self.event_manager
                )
...
def create_plugin_manager(
    config: dict[str, Any] | None = None, logger: logging.Logger | None = None
) -> PluginManager:
