...
class TestServiceWithDependencies:
    ...
    def test_get_dependency_success(self, mock_container, mock_config):
        ...
        mock_container.get.side_effect = (
            lambda type_: mock_dependency if type_ == TestDependency else None
        )
    ...
    @pytest.mark.asyncio
    async def test_service_with_dependencies_lifecycle(self, mock_container, mock_config):
...
class TestServiceInheritance:
    ...
    def test_concrete_service_inherits_base_functionality(self, mock_container, mock_config):
