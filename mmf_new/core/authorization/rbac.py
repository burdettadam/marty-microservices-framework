...
- Fine-grained permission system with wildcard support
- Circular dependency detection in role hierarchies
- Permission caching for performance optimization
- Default system roles (admin, service_manager, developer, viewer, service_account)
- Configuration-based role loading and export
- User role assignment and permission checking

Architecture:
- Permission: Fine-grained access control unit (resource:id:action)
- Role: Named collection of permissions with inheritance
- RBACManager: Central manager for roles, users, and permission evaluation
- RBACManagerService: DI-compatible service wrapper
"""
...
from marty_msf.security_core.exceptions import (
    PermissionDeniedError,
    RoleRequiredError,
)

from .api import Permission, PermissionAction, ResourceType
...
class Role:
    ...
    A role is a named collection of permissions that can inherit from other roles.
    Roles form a hierarchy through the inherits_from relationship, enabling
    permission composition and reuse.

    Attributes:
        name: Unique role identifier
        description: Human-readable description
        permissions: Set of permissions granted by this role
        inherits_from: Set of parent role names to inherit permissions from
        metadata: Additional role metadata
        created_at: Role creation timestamp
        is_system: If True, role cannot be deleted (protected system role)
        is_active: If False, role cannot be assigned to new users
    """
    ...
    def has_permission(self, resource_type: str, resource_id: str, action: str) -> bool:
        """
        Check if role directly grants specific permission.

        Note: Does not check inherited permissions. Use RBACManager for
        full permission resolution with inheritance.
        """
...
class RBACManager:
    ...
    Central manager for role-based access control, handling:
    - Role lifecycle (create, update, delete)
    - User-role assignments
    - Permission evaluation with inheritance
    - Role hierarchy management with cycle detection
    ...
    Performance:
        - Permission checks are cached per user with configurable TTL
        - Role hierarchy is pre-computed and cached
    ...
    def __init__(self):
        ...
        self.role_hierarchy: dict[str, set[str]] = {}  # role -> inherited roles (flattened)
    ...
    def _initialize_default_roles(self):
        ...
        - service_manager: Service and config management
        - developer: Read access with limited write
        - viewer: Read-only non-sensitive resources
        - service_account: Limited automated system access
        """
        # Super admin role
        admin_role = Role(
    ...
    def add_role(self, role: Role) -> bool:
        ...
        - Parent roles exist
        - No circular inheritance created
        ...
        Raises:
            ValueError: If role name exists, parent missing, or creates cycle
        """
        try:
    ...
    def remove_role(self, role_name: str) -> bool:
        ...
        System roles cannot be removed for safety. When removed:
        - Role is removed from all users
        - Other roles lose inheritance from this role
        ...
        Raises:
            ValueError: If attempting to remove system role
        """
        try:
    ...
    def assign_role_to_user(self, user_id: str, role_name: str) -> bool:
        ...
        Raises:
            ValueError: If role doesn't exist or is not active
        """
        try:
            if role_name not in self.roles:
    ...
    def remove_role_from_user(self, user_id: str, role_name: str) -> bool:
        ...
        Returns:
            True if removed, False if user had no roles
        """
        try:
    ...
    def check_permission(
        ...
        Evaluates all user's effective permissions (direct + inherited)
        against the requested access.

        Args:
            user_id: User identifier
            resource_type: Type of resource (e.g., "service", "config")
            resource_id: Resource identifier or wildcard
    ...
    def check_role(self, user_id: str, role_name: str) -> bool:
        ...
        Returns:
            True if user has role directly or through inheritance
        """
    ...
    def get_user_roles(self, user_id: str) -> set[str]:
        ...
        Does not include inherited roles. Use get_user_effective_roles()
        for complete role set.
        """
    ...
    def _get_user_effective_roles(self, user_id: str) -> set[str]:
        ...
        Flattens role hierarchy to return complete set of
        directly assigned and inherited roles.
        """
    ...
    def _get_user_permissions(self, user_id: str) -> set[Permission]:
        ...
        Caches computed permissions per user for performance.
        Cache is invalidated on role/assignment changes.
        """
    ...
    def _update_role_hierarchy(self, role: Role):
        ...
        Computes and caches the complete set of inherited roles
        for the given role by traversing the inheritance chain.
        """
        inherited_roles = set()
    ...
    def _rebuild_role_hierarchy(self):
        """Rebuild entire role hierarchy after structural changes."""
    ...
    def _would_create_cycle(self, role_name: str, parent_role: str) -> bool:
        ...
        Uses depth-first search to detect cycles in the role
        inheritance graph before adding new inheritance edge.

        Args:
            role_name: Role being added/modified
            parent_role: Proposed parent role

        Returns:
            True if adding inheritance creates cycle
        """
        visited = set()
    ...
    def load_roles_from_config(self, config_data: dict[str, Any]) -> bool:
        ...
        Config format:
        {
            "roles": {
                "role_name": {
                    "description": "...",
                    "permissions": ["resource:id:action", ...],
                    "inherits": ["parent_role", ...]
                }
            }
        }

        System roles are skipped for safety.
    ...
    def export_roles_to_config(self) -> dict[str, Any]:
        ...
        System roles are excluded from export.

        Returns:
            Configuration dictionary with non-system roles
        """
        roles_data = {}
    ...
    def get_role_info(self, role_name: str) -> dict[str, Any] | None:
        ...
        Includes effective permissions from inheritance.

        Args:
            role_name: Role to inspect

        Returns:
            Role information dictionary or None if not found
        """
        if role_name not in self.roles:
            return None
    ...
    def _get_role_effective_permissions(self, role_name: str) -> set[Permission]:
        ...
        Returns:
            Set of all permissions from role and inherited roles
        """
        permissions = set()
    ...
    def list_roles(self, include_system: bool = False) -> list[dict[str, Any]]:
        ...
        Args:
            include_system: If True, include system roles in list
...
class RBACManagerService:
    ...
    Provides DI container integration for RBACManager.
    Registered as singleton in the DI container.
    """

    def __init__(self):
        """Initialize service with new RBACManager instance."""
...
def reset_rbac_manager():
    """
    Reset RBAC manager (not supported).

    RBAC manager lifecycle is controlled by the DI container.
