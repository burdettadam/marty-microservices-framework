...
class ModelServer(ModelServingPort):
    ...
    def __init__(
        ...
        self.metrics_collector = metrics_collector or PrometheusCollector()
    ...
    def _init_metrics(self):
        ...
        self.metrics_collector.register_metric(
            MetricDefinition(
                name="ml_model_cache_hits_total",
                metric_type=MetricType.COUNTER,
                description="Total number of prediction cache hits",
                labels=["model_id"],
            )
        )
    ...
    async def predict(
        ...
        try:
            ...
            if use_cache:
                ...
                if cached_prediction:
                    await self.metrics_collector.increment_counter(
                        "ml_model_cache_hits_total", labels={"model_id": model_id}
                    )
            ...
            # Update metrics
            self._update_model_metrics(model_id, latency * 1000, success=True)
            await self.metrics_collector.increment_counter(
                "ml_model_predictions_total", labels={"model_id": model_id, "status": "success"}
            )
            await self.metrics_collector.observe_histogram(
                "ml_model_prediction_latency_seconds", latency, labels={"model_id": model_id}
            )
            ...
            latency = time.time() - start_time
            self._update_model_metrics(model_id, latency * 1000, success=False)
            await self.metrics_collector.increment_counter(
                "ml_model_predictions_total", labels={"model_id": model_id, "status": "error"}
            )
            await self.metrics_collector.increment_counter(
                "ml_model_errors_total",
                labels={"model_id": model_id, "error_type": type(e).__name__},
            )
    ...
    async def _make_prediction(
        ...
        # Generic prediction
        return {"prediction": float(np.random.random()), "confidence": float(np.random.random())}
