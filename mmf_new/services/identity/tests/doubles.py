...
from mmf_new.services.identity.domain.models import AuthenticatedUser, UserId, Credentials
...
class InMemoryUserRepository(UserRepository, Repository[AuthenticatedUser]):
    ...
    def find_by_username(self, username: str) -> Optional[UserId]:
    ...
    async def find_by_id(self, entity_id: UUID | str | int) -> Optional[AuthenticatedUser]:
        return None
    ...
    async def update(self, entity_id: UUID | str | int, updates: dict[str, Any]) -> Optional[AuthenticatedUser]:
        return None
