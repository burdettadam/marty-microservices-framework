...
def create_secret_manager(provider: ConfigProvider) -> SecretManager:
    """Create secret manager and register it in DI container."""

    container = get_container()
    secret_manager = SecretManager(provider)
    container.register_instance(SecretManager, secret_manager)
    return secret_manager


def get_secret_manager() -> SecretManager | None:
    """Get secret manager from DI container."""

    container = get_container()
    try:
        return container.get(SecretManager)
    except Exception:
        return None
