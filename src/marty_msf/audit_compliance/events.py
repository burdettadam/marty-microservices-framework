...
class SecurityEventManager:
    ...
    def log_authorization_event(
        ...
        event_type = SecurityEventType.AUTHORIZATION_FAILURE if not allowed else SecurityEventType.DATA_ACCESS
    ...
    def get_event_summary(
        ...
        summary["top_source_ips"] = dict(sorted(ip_counts.items(), key=lambda x: x[1], reverse=True)[:10])

        # Threat indicators
        high_severity_events = [e for e in recent_events if e.severity in (SecurityEventSeverity.HIGH, SecurityEventSeverity.CRITICAL)]
        if high_severity_events:
            summary["threat_indicators"].append(f"{len(high_severity_events)} high/critical severity events")

        failed_auth_events = [e for e in recent_events if e.event_type == SecurityEventType.AUTHENTICATION_FAILURE]
        if len(failed_auth_events) > 10:
            summary["threat_indicators"].append(f"{len(failed_auth_events)} authentication failures")
    ...
    def clear_events(self, before: datetime | None = None) -> int:
        ...
        if before:
            ...
            self.events = deque(
                [e for e in self.events if e.timestamp >= before],
    ...
    def _check_threat_pattern(
        ...
        related_events = [
            ...
            if (e.timestamp >= cutoff and
                e.event_type in pattern_config["event_types"] and
                severity_levels.get(e.severity, 0) >= threshold_level)
        ]
