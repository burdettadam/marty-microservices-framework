...
class BasicAuthenticator(IAuthenticator):
    ...
    def authenticate(self, credentials: builtins.dict[str, Any]) -> AuthenticationResult:
        ...
        if not username or not password:
            return AuthenticationResult(success=False, error="Username and password required")
        ...
        return AuthenticationResult(success=True, user=user, metadata={"auth_method": "basic"})

    def validate_token(self, token: str) -> AuthenticationResult:
        """Basic authenticator doesn't use tokens."""
        return AuthenticationResult(
            success=False, error="Token validation not supported for basic auth"
        )

    def refresh_token(self, refresh_token: str) -> AuthenticationResult:
        """Token refresh not supported."""
        return AuthenticationResult(success=False, error="Token refresh not supported")
...
class JwtAuthenticator(IAuthenticator):
    ...
    def authenticate(self, credentials: builtins.dict[str, Any]) -> AuthenticationResult:
        ...
        payload = {"sub": username, "iat": datetime.now(timezone.utc), "exp": expires_at}
        ...
        return AuthenticationResult(
            success=True, user=user, metadata={"auth_method": "jwt", "access_token": token}
        )
    ...
    def validate_token(self, token: str) -> AuthenticationResult:
        ...
        try:
            ...
            return AuthenticationResult(
                success=True, user=user, metadata={"auth_method": "jwt", "token_validated": True}
            )

        except jwt.ExpiredSignatureError:
            return AuthenticationResult(success=False, error="Token has expired")
...
class TokenAuthenticator(IAuthenticator):
    ...
    def authenticate(self, credentials: builtins.dict[str, Any]) -> AuthenticationResult:
        ...
        if not token:
            return AuthenticationResult(success=False, error="Token required")
    ...
    def validate_token(self, token: str) -> AuthenticationResult:
        ...
        if not token_data.get("active", True):
            return AuthenticationResult(success=False, error="Token is disabled")
        ...
        return AuthenticationResult(success=True, user=user, metadata={"auth_method": "token"})
...
class MultiFactorAuthenticator(IAuthenticator):
    ...
    def __init__(
        self, primary_authenticator: IAuthenticator, secondary_authenticator: IAuthenticator
    ):
    ...
    def authenticate(self, credentials: builtins.dict[str, Any]) -> AuthenticationResult:
        ...
        if not secondary_result.success:
            return AuthenticationResult(success=False, error="Second factor authentication failed")
...
class MTLSAuthenticator(IAuthenticator):
    ...
    def authenticate(self, credentials: dict[str, Any]) -> AuthenticationResult:
        ...
        if not cert_data:
            return AuthenticationResult(
                success=False, error="Client certificate required", error_code="MISSING_CLIENT_CERT"
            )
    ...
    def validate_certificate(self, cert) -> AuthenticationResult:
        ...
        try:
            ...
            if cert.not_valid_after.replace(tzinfo=timezone.utc) < now:
                return AuthenticationResult(
                    success=False, error="Certificate has expired", error_code="CERT_EXPIRED"
                )
