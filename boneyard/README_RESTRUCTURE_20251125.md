...
│   ├─ infrastructure/
│   │ ├─ adapters/
│   │ │ ├─ in/                   # Driving Adapters (API, CLI, Consumers)
│   │ │ └─ out/                  # Driven Adapters (DB, External APIs)
│   │ └─ config/                 # Service-specific config
│   └─ main.py                   # Wiring / Entrypoint
│
├─ core/ (The "Platform Core")
│ ├─ contracts/                  # Abstract Interfaces (Entity, Repository)
│ └─ policies/                   # Global Policy Definitions
│
└─ framework/ (Optional Shared Libs)
  ├─ security/
  └─ observability/
```
...
- **Tasks**:
  ...
  - Create database and HTTP adapters (split into `in/` and `out/`)
