...
## New Testing Framework Core (`mmf_new/core/testing`)

The core testing framework has been migrated to `mmf_new/core/testing` and restructured using Hexagonal Architecture.

### Structure

- **Domain (`mmf_new/core/testing/domain/`)**: Defines core test entities, enums, and domain logic for Chaos, Contract, and Performance testing.
- **Application (`mmf_new/core/testing/application/`)**: Contains test runners and orchestrators (`ChaosRunner`, `ContractVerifier`, `PerformanceRunner`).
- **Infrastructure (`mmf_new/core/testing/infrastructure/`)**: Provides adapters for databases, event buses, and pytest integration.
- **API (`mmf_new/core/testing/api/`)**: Exposes the public API for test authors (`AsyncTestCase`, `ServiceTestMixin`).

### Usage

To use the new framework in your tests:

```python
from mmf_new.core.testing.api import AsyncTestCase, ServiceTestMixin
...
The pytest plugin is automatically registered and provides fixtures like `test_database` and `test_session`.
