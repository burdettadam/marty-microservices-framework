...
class FastStreamConsumer(IMessageConsumer):
    ...
    async def start(self) -> None:
        ...
        self._running = True
        # Note: FastStream usually uses decorators for consumers.
        # To support dynamic consumers, we might need to use the broker's low-level API
        # or register a subscriber dynamically.

        if self._handler:
            # Dynamic subscription
            self.broker.include_router(self._create_router())
...
class FastStreamBackend(IMessageBackend):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            elif self.config.type == BackendType.PUBSUB:  # NATS
