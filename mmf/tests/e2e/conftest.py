...
from mmf.framework.observability.adapters.monitoring import ServiceMonitor
from mmf.framework.observability.monitoring import InMemoryCollector
from mmf.framework.testing import PerformanceTestCase, TestEventCollector
...
class TimeoutMonitor:
    ...
    async def monitor_operation(self, operation_name: str, operation_func, *args, **kwargs):
        ...
        try:
            ...
            # Return result wrapped with duration for analysis
...
class SimulationPlugin:
    """Plugin for simulating work in tests."""

    def __init__(self):
        self.config = SimulationConfig()

    async def simulate_work(self, task_name: str, complexity: float = 1.0):
...
@pytest.fixture
def simulation_plugin():
    """Provides a simulation plugin for tests."""
    return SimulationPlugin()


class PipelinePlugin:
    """Plugin for pipeline operations."""
...
@pytest.fixture
def circuit_breaker_plugin():
    """Provides a circuit breaker plugin for tests."""
...
class InMemoryEventBus(EventBus):
    ...
    async def publish_batch(
        self,
        events: builtins.list[BaseEvent],
        delivery_guarantee: DeliveryGuarantee = DeliveryGuarantee.AT_LEAST_ONCE,
    ) -> None:
        for event in events:
