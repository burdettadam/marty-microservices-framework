...
from mmf.services.identity.infrastructure.adapters.out.mfa.sms_mfa_adapter import (
    SMSMFAAdapter,
    SMSMFAConfig,
)


class TestSMSMFAAdapter:
    @pytest.fixture
    def config(self):
        return SMSMFAConfig(
    ...
    @pytest.mark.asyncio
    async def test_validate_phone_number(self, adapter):
        assert await adapter.validate_phone_number("+1234567890") is True
        assert await adapter.validate_phone_number("1234567890") is False  # Missing +
        assert await adapter.validate_phone_number("invalid-phone") is False

    @pytest.mark.asyncio
    async def test_register_device(self, adapter):
        device_data = {"phone_number": "+1234567890"}
        device = await adapter.register_device(
            "user123", MFADeviceType.SMS_PHONE, "My Phone", device_data
        )

        assert device.user_id == "user123"
        assert device.device_type == MFADeviceType.SMS_PHONE
    ...
    async def test_verify_device(self, adapter):
        # Register device
        device = await adapter.register_device(
            "user123", MFADeviceType.SMS_PHONE, "My Phone", {"phone_number": "+1234567890"}
        )
    ...
    async def test_create_challenge(self, adapter):
        ...
        challenge = await adapter.create_challenge(
            "user123", MFAMethod.SMS, device_id=device.device_id
        )
    ...
    async def test_verify_challenge_success(self, adapter):
        ...
        assert response.metadata["method"] == "sms"
