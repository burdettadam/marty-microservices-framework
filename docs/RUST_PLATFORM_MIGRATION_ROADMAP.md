...
| Security | Active; Python MFA and OAuth2/OIDC implementations deleted | `mmf-security` now owns authenticated-user/principal/result models; structured permissions; protected default roles, role lifecycle, assignment and inherited RBAC; nested/regex deny-overrides ABAC; conditioned user/role/group ACLs with glob, time, CIDR, method and resource-attribute matching; composite authorization; deterministic fixed/sliding/token/leaky rate limiting; session lifecycle and hijack policy; audit/threat/vulnerability/mesh models; and explicit session/KMS/OpenBao/OPA/Oso provider ports. It is the single implementation of MFA challenge/device/verification lifecycle, RFC 6238 SHA-1/SHA-256/SHA-512 TOTP, bounded time windows, replay and rate-limit enforcement, one-use hashed backup codes, delivery provider ports, and the complete OAuth2/OIDC model and port surface. OAuth2/OIDC now includes all intended client profiles and registration/lifecycle operations; authorization, PKCE, token, introspection and in-memory store behavior; complete provider/store contracts; discovery and provider capabilities; ID token/UserInfo/authentication request models; JWK/JWKS parsing/cache; componentized JWT validation with an explicit signature-verifier port; exact redirect-authority checks; and fail-closed malformed, expired, unknown-key, missing-verifier and unsupported-algorithm handling. Rust and the deleted Python oracles passed `contracts/identity-mfa-behavior.json` and `contracts/identity-oauth-oidc-behavior.json`; 2,154 production Python MFA lines, 1,069 implementation-specific MFA test lines, and 3,437 production Python OAuth2/OIDC lines were removed after the maintained suite passed (1,615 passed, 1 skipped). The Rust port also repairs the already-broken Python service-client constructor and standalone OIDC imports. External policy, MFA delivery, JWT signature and identity providers fail closed rather than reporting placeholder success. Enforcement middleware/provider adapters and the remaining consumer cutovers are still pending. |
...
| Built-in identity service | Active; MFA and OAuth2/OIDC subdomains deleted | `mmf-services::identity` is consolidating the Python identity family around canonical `mmf-security` user/session/MFA/OAuth2/OIDC models; it owns stable authentication status/error behavior and fail-closed token/password/API-key/mTLS provider composition. MFA, OAuth2 and OIDC are re-exported from `mmf-security`, and the older service-local Rust OAuth client/authorization-code implementation has been deleted rather than maintained as a second implementation. Python and Rust execute `contracts/identity-service-behavior.json`; Rust directly executes the OAuth2/OIDC fixture. Session, mTLS, configuration, authentication adapter and HTTP surfaces are the remaining identity deletion gates. |
...
| Plugin platform | Active; consumer cutover pending | `mmf-plugins` now owns service and gateway-middleware plugin kinds, metadata/context/routes/services, namespaced typed configuration and provider ports, discovery and loader ports, registry conflict checks, missing-dependency and cycle rejection, deterministic dependency/start/reverse-stop ordering, lifecycle state enforcement, dynamic enable/disable, service status, health, event subscriptions, and safe unload rules. Rust executes `contracts/plugins-behavior.json`; Python executes the surviving model/config vectors. The local Python manager facade currently imports a missing `mmf.framework.plugins` implementation, so Rust repairs the documented intended behavior. Python core/config deletion waits for the identity HTTP adapter and verification scripts to cut over. |
