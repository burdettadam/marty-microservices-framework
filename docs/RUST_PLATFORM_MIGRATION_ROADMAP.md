...
| Security | Active; Python MFA implementation deleted | `mmf-security` now owns authenticated-user/principal/result models; structured permissions; protected default roles, role lifecycle, assignment and inherited RBAC; nested/regex deny-overrides ABAC; conditioned user/role/group ACLs with glob, time, CIDR, method and resource-attribute matching; composite authorization; deterministic fixed/sliding/token/leaky rate limiting; session lifecycle and hijack policy; audit/threat/vulnerability/mesh models; and explicit JWT/OIDC/session/KMS/OpenBao/OPA/Oso provider ports. It is also the single implementation of MFA challenge/device/verification lifecycle, RFC 6238 SHA-1/SHA-256/SHA-512 TOTP, bounded time windows, replay and rate-limit enforcement, one-use hashed backup codes, SMS/email code delivery ports, and push/hardware/voice provider ports. Rust and the deleted Python oracle passed `contracts/identity-mfa-behavior.json`; 2,154 production Python MFA lines and 1,069 implementation-specific test lines were removed after the maintained unit/contract suite passed. External policy and MFA delivery providers fail closed rather than reporting placeholder success. Enforcement middleware/provider adapters and the remaining consumer cutovers are still pending. |
...
| Built-in identity service | Active; MFA subdomain deleted | `mmf-services::identity` is consolidating the Python identity family around canonical `mmf-security` user/session/MFA models; it owns stable authentication status/error behavior, provider orchestration, OAuth2 client and one-time authorization-code state, and fail-closed token/password/API-key/mTLS provider composition. MFA is now re-exported from `mmf-security`, not duplicated in the service crate. Python and Rust execute `contracts/identity-service-behavior.json`; the remaining OAuth2/OIDC, session, mTLS, configuration, authentication adapter and HTTP surfaces are the next identity deletion gates. |
