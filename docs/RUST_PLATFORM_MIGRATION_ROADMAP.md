...
| Security | Active; Python MFA, OAuth2/OIDC, mTLS, and identity-session implementations deleted | `mmf-security` owns authenticated-user/principal/result models; authorization and rate limiting; audit/threat/mesh models; MFA/TOTP; OAuth2/OIDC; reusable mTLS; and canonical session lifecycle, configuration and events. The session surface includes deterministic idle/absolute expiry and extension, context matching, activity/data/role/permission state, suspension/invalidation/termination, secure identifiers, every intended identity event type, typed event details and batches, complete storage/cleanup/security-level factories, redacted connection strings, and explicit store/event/analytics/archive ports. The older core-session record remains as a compatibility adapter for live memory/Redis consumers until their cutover, while new behavior is implemented once in Rust. Rust and deleted Python oracles passed language-neutral identity contracts; removed Python includes 2,154 MFA lines, 3,437 OAuth2/OIDC lines, 1,566 physical mTLS lines, and 1,519 physical/1,246 nonblank identity-session lines. Rust also repairs invalid external-storage factories by requiring connection strings and rejects executable pickle session serialization. External effects fail closed. Remaining Python core-session adapters, enforcement adapters, and consumer cutovers are pending. |
...
| Built-in identity service | Active; MFA, OAuth2/OIDC, mTLS, and identity-session subdomains deleted | `mmf-services::identity` composes and re-exports canonical `mmf-security` user, MFA, OAuth2/OIDC, mTLS, managed-session, session-configuration and session-event models and provider contracts. Duplicate service-local OAuth/mTLS surfaces and all isolated Python identity security subdomains are deleted. Python and Rust execute language-neutral identity contracts. General service configuration, authentication adapters/use cases, persistence and HTTP surfaces are the remaining identity deletion gates; live core-security memory/Redis session adapters are tracked separately as a consumer cutover. |
