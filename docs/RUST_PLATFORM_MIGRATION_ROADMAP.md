...
| Security | Active; Python MFA, OAuth2/OIDC, mTLS, identity-session, and built-in identity cryptography deleted | `mmf-security` owns authenticated-user/principal/result models; authorization and rate limiting; audit/threat/mesh models; MFA/TOTP; OAuth2/OIDC; reusable mTLS; canonical session lifecycle, configuration and events; and the first-party HS256/384/512 JWT codec. JWT verification pins type and algorithm, checks signatures in constant time, requires issuer/audience/time/JTI claims, and rejects reserved-claim overrides. The older core-session record remains as a compatibility adapter for live memory/Redis consumers until their cutover, while new behavior is implemented once in Rust. Rust and deleted Python oracles passed language-neutral identity contracts; removed Python includes 2,154 MFA lines, 3,437 OAuth2/OIDC lines, 1,566 physical mTLS lines, 1,519 physical/1,246 nonblank identity-session lines, plus the built-in identity deletion recorded below. Rust also repairs invalid external-storage factories by requiring connection strings and rejects executable pickle session serialization. External effects fail closed. Remaining Python core-session adapters, enforcement adapters, and consumer cutovers are pending. |
