...
| Security | Active | `mmf-security` now owns authenticated-user/principal/result models, hierarchical RBAC, deny-overrides ABAC, deterministic fixed/sliding/token/leaky rate limiting, session lifecycle and hijack policy, audit/threat/vulnerability/mesh models, and explicit JWT/OIDC/session/MFA/KMS/OpenBao provider ports; cryptographic kernels remain in `marty-core`, required providers fail startup closed, and Python/Rust execute `contracts/security-behavior.json` |
| Data and infrastructure | Active | `mmf-data` now owns cache backend/pattern/config/stats semantics, fail-closed JSON/string/bytes serialization, full ordinary and sorted-set cache ports, deterministic TTL behavior, read-model equality/range/set filtering, validated sorting/pagination, generic repositories, migration revisions/schema verification, and validated PostgreSQL identifier/literal/JSONB/insert generation. Executable Python pickle is intentionally rejected rather than ported. Redis/Memcached/SQL/migration adapters, DI/runtime consolidation, unified-config provider cutover, and consumers remain before deleting the Python infrastructure family; Python and Rust execute `contracts/data-infrastructure-behavior.json`. |
