...
| Discovery, gateway, mesh and deployment | Active; proxy and HTTP kernels complete | `mmf-platform` consolidates service endpoints/metadata/health, filtered discovery, all seven load-balancing strategies, route matching/constraints, deployment lifecycle and Kubernetes manifests. Wave 25 adds the canonical reverse-proxy executor over existing discovery, upstream and `mmf-resilience` ports: bounded request/response bodies, route deadlines, idempotent-method retries only, trusted identity-header stripping/reinjection, repeated query values with legacy incoming-key precedence, path rewriting, response-header filtering, and MIP error normalization. The same crate now owns reusable protocol-version negotiation, content-type policy, weak ETag decisions, exact request fingerprints, provider-neutral idempotency leases/stores, and a deterministic in-memory store with replay, conflict, in-progress, expiry, completion and safe-abort semantics. Missing healthy instances and providers fail closed; timeout and transport failures are typed. `contracts/platform-behavior.json`, `contracts/gateway-runtime-behavior.json`, and `contracts/http-runtime-behavior.json` are the language-neutral oracles. Mesh policy models remain reused from `mmf-security`; rate limiting remains canonical in that crate. Marty UI supplies only its MIP policy values plus Redis/gRPC adapters. Gateway cutover and obsolete Python deletion are the next gate. |
...
| `mmf-platform` | Service discovery and registration, Consul/DNS adapters, load balancing, route matching and reverse-proxy execution, protocol-version/content-type/ETag/idempotency HTTP kernels, service-mesh policy/configuration, Kubernetes/container metadata, and deployment generation |
