...
| Security | Active; Python MFA, OAuth2/OIDC, and mTLS implementations deleted | `mmf-security` now owns authenticated-user/principal/result models; authorization and rate limiting; session policy; audit/threat/mesh models; MFA/TOTP; OAuth2/OIDC; and the complete reusable mTLS model, configuration, policy, authentication, session, audit-event, extraction, identity-mapping, trust-store, revocation and provider-contract surface. The mTLS implementation includes every extension model promised by the former Python public API, strict one-label wildcard matching, deterministic validity/session behavior, redacted trust-store secrets, explicit trust material, complete file/PKCS#11/development/production/high-security configuration, and fail-closed provider composition. X.509 parsing, path/signature verification, CRL and OCSP cryptography remain explicit `marty-core` provider effects instead of duplicate MMF crypto. Rust and the deleted Python oracles passed the language-neutral identity contracts; 2,154 production Python MFA lines, 3,437 production Python OAuth2/OIDC lines, and all 1,566 physical/1,262 nonblank Python mTLS lines were removed after parity and consumer gates. The Rust ports also repair the broken Python OAuth/OIDC imports, invalid service-client constructor, missing mTLS domain types, invalid high-security PKCS#11 factory, absent validation trust flag, over-broad wildcard matching, and empty-trust default factory. External effects fail closed rather than reporting placeholder success. Enforcement adapters and remaining consumer cutovers are still pending. |
...
| Built-in identity service | Active; MFA, OAuth2/OIDC, and mTLS subdomains deleted | `mmf-services::identity` now composes and re-exports canonical `mmf-security` user/session/MFA/OAuth2/OIDC/mTLS models and provider contracts. The duplicate service-local OAuth code and thin service-local mTLS provider trait were deleted, leaving one Rust implementation surface and stable service orchestration for token, password, API-key and native mTLS providers. Python and Rust execute language-neutral identity contracts. Session, configuration, authentication-adapter, persistence and HTTP surfaces are the remaining identity deletion gates. |
