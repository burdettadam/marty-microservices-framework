...
Every service under `mmf/services/<service_name>/` MUST have this structure:
...
Every framework module under `mmf/framework/<module_name>/` MUST have this structure:
...
All service containers MUST inherit from `mmf.core.di.BaseDIContainer`:
...
def test_domain_has_no_infrastructure_imports():
    ...
    pytest_archon.assert_no_import(
        source="mmf.services.*.domain",
        target="mmf.services.*.infrastructure"
    )
...
- **Service**: `mmf/services/audit/` - Complete hexagonal architecture with DI container
- **Framework Module**: `mmf/framework/gateway/` - Proper layering in framework code
